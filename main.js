...
function updateUIForLimits() {
  ...
  if (userLimits.canUse) {
    ...
    // Mostrar mensaje integrado al chat, con botón y leyenda
    emptyState.innerHTML = `
      <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 36px 0 20px 0;">
        ...
        <button onclick="iniciarPago()" class="boton-suscripcion-integrado" style="margin:12px auto 0 auto;display:flex;align-items:center;gap:8px;">
          <img src="imagenes/mercadopago.png" style="height:32px;width:auto;margin-right:8px;" alt="Suscribite con Mercado Pago">
          Suscribirme ahora
        </button>
        <div style="margin-top:14px;font-size:0.96rem;opacity:0.7;">Suscribite por $1999/mes para uso ilimitado</div>
      </div>
    `;
    // OCULTÁ el botón flotante si sigue existiendo
    const botonSuscripcion = document.getElementById('botonSuscripcion');
    if (botonSuscripcion) botonSuscripcion.style.display = 'none';
